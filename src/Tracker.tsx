...
export default function Tracker() {
    ...
    (async () => {
      try {
        ...
        if (saved) {
          ...
          await storageService.save(SEEDED_LOGS);
        }
      } catch {
        setLogs(SEEDED_LOGS);
        setActiveDay(SEEDED_LOGS.length - 1);
  ...
  const addDay = useCallback(() => {
    ...
    clearTimeout(timerRef.current);
    pendingRef.current = null;
    storageService.save(newLogs); // immediate — deliberate action
  }, [logs]);

  const updateDay = useCallback((index, updated) => {
    const newLogs = logs.map((l, i) => i === index ? updated : l);
    setLogs(newLogs);
    debouncedSave(newLogs); // debounced — may be typing
  }, [logs, debouncedSave]);
  ...
  const TABS    = ["📋 Дневник", "📈 Прогресс", "📊 Аналитика", "📌 План"];
